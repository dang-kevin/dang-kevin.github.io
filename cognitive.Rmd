...
data %>% group_by(distraction) %>% summarize(mean.ontime.minus.offtime = round(mean(diff), 2)) %>% 
...
data %>% group_by(order) %>% summarize(mean.ontime.minus.offtime = round(mean(diff), 2)) %>%
  kable(caption = "Order means")
```

```{r tab4, message=F, warning=F, echo=F}
data %>% group_by(distraction, order) %>% summarize(mean.ontime.minus.offtime = round(mean(diff), 2), count = n()) %>%
  kable(caption = "Distraction by Order means")
```

```{r tab5, message=F, warning=F, echo=F}
data %>% group_by(device) %>% summarize(mean.ontime.minus.offtime = round(mean(diff), 2)) %>% 
